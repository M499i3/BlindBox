      ...
      <div className="pt-16 pb-24 overflow-y-auto flex-1 px-container-margin">
        {ctx && (
          <section className="py-stack-md sticky top-0 z-40">
            <div className="glass-card rounded-xl p-3 flex items-center justify-between shadow-lg gap-2">
              ...
              {ctx.listingId && !ctx.orderId ? (
                <button
                  ...
                  className="flex-shrink-0 text-[10px] font-bold text-primary px-2 py-1 rounded-lg border border-primary/30"
                >
                  {ordering ? '處理中…' : '下單'}
                </button>
              ) : null}
          ...
          {messages.map((msg) =>
            msg.type === 'system' ? (
              <div key={msg.id} className="flex justify-center">
                <span className="text-[10px] text-on-surface-variant bg-neutral-100 px-3 py-1 rounded-full">
                  {msg.content}
                </span>
              </div>
            ) : msg.isMine ? (
